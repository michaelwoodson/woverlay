...
class Overlay {
	...
	loadFlood(connections) {
		this.flood = Overlay.makeFlood(connections, this.localid);
	}
...
`Failed to proxy: ${envelope.overlay} localid: ${idhelper.shortId(this.localid.id, false)}
  ...
  active: ${activeConnectionIds.map(id => idhelper.shortId(id, false))}`
  				);
		...
		if ('findfinger' === envelope.overlay) {
			...
			if (signed.level >= this.fingers.length) {
				...
				if (envelope.overlayFrom != this.fingers[signed.level]) {
					console.log(`should pick best finger: ${idhelper.shortId(envelope.overlayFrom)} != ${idhelper.shortId(this.fingers[signed.level])}`);
					this.fingers[signed.level] = this.bestFinger(localid.id, signed.level, envelope.overlayFrom, this.fingers[signed.level]);
				}
			} else {
