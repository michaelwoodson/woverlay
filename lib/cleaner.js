...
class Cleaner {
	...
	checkFloodAndFingers(retry) {
		let goldenIds = this.overlay.getGoldenIds();
		//let targetFlood = this.overlay.targetFlood();
		let flood = this.overlay.flood;
		// Don't worry about fingers unless the flood network is full.
		if (flood.length == this.overlay.totalFloodSize()) {
			// Check to see if the fingers should grow.
			let distance = idhelper.directedDistance(flood[0], flood[flood.length -1]);
			...
			if (this.overlay.fingers.length && distance.compareTo(idhelper.HALF_MAX.shiftRight(this.overlay.fingers.length-1)) > 0) {
				console.log(`time to remove a finger. length: ${this.overlay.fingers.length}`);
