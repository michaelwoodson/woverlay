...
class WebSocketWrapper {
	constructor(overlay, host) {
		...
		this.host = host;
		this.retrySeconds = 1;
		this.successfulInitiateSet = new Set();
	...
	addConnection(connection) {
		let compositeId = connection.peerId + connection.instanceId;
		if (connection.peerId === this.localid.id) {
			...
			if (connection.initiator) {
				this.send(
					...
					this.successfulInitiateSet.has(compositeId) ? 'accepted' : 'youinitiate',
					this.connectInfoMap[compositeId]
				);
			} else {
				this.successfulInitiateSet.add(compositeId);
			}
		}
	}
}
