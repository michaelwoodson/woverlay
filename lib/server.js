...
/**
 * The main server, handles the landing page and websocket signalling.
 */
let app = express();
...
wss.on('connection', function connectionHandler(ws) {
	...
	ws.onmessage= function onmessage(event) {
		...
		if (data.action === 'bootstrap' || data.action === 'reconnect') {
			...
			if (ws2 && ws2.instanceId == ws.instanceId) {
...
function removeSocket(ws) {
	...
	if (ws.readyState === 2 || ws.readyState === 3) {
		...
		console.log('removing in 10: ' + ws.id);
		setTimeout(function timeoutSocket() {
			console.log('timed out: ' + ws.id);
			removeFromMaps(ws);
			try {
				ws.close();
			} catch (error) {
...
function showStatus() {
	console.log('Probation: ' + status.probationSocketHolder.sockets.length + " Verified: " + status.verifiedSocketHolder.sockets.length);
