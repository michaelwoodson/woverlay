		...
		ws.onmessage= function onmessage(event) {
			try {
				...
				if (listeners.has(data.action)) {
					...
					if (differentInstanceAlreadyConnected(data)) {
						console.log('different instance already connected ' + data.action);
						// Will have till the timeout to connect. (~10 seconds)
						removeSocket(ws);
						serverStatus.subordinateSocketMap[data.fromInstance.guid] = ws;
						ws.subordinate = true;
						...
						if (data.action === 'bootstrap') {
							...
							if (serverStatus.verifiedSocketHolder.sockets.length > 0 && serverStatus.verifiedSocketHolder.sockets.length < MAX_SOCKETS) {
								console.log('sending checkforid request');
					...
					pingCount++;
					send(ws, JSON.stringify({action: 'pong'}));
				} else if (data.to && ws.id) {
					ws2 = serverStatus.socketMap[data.to];
					if (ws2 && ws.id == ws2.id && ws2.instanceId.guid == ws.instanceId.guid) {
						ws2 = serverStatus.subordinateSocketMap[data.toInstance.guid];
					}
					if (ws2) {
						send(ws2, event.data);
					} else {
					...
					console.log('weird message, removing: ' + JSON.stringify(data));
					removeSocket(ws);
				}
			} catch (e) {
				console.log('error for message: ' + JSON.stringify(event.data));
				console.log('e: ' + e);
			}
		};
...
function differentInstanceAlreadyConnected(data) {
	return data.from in serverStatus.socketMap && data.fromInstance.guid !== serverStatus.socketMap[data.from].instanceId.guid;
}
...
function removeFromMaps(ws) {
	...
	if (ws.instanceId && ws.instanceId.guid in serverStatus.subordinateSocketMap && serverStatus.subordinateSocketMap[ws.instanceId.guid] == ws) {
		delete serverStatus.subordinateSocketMap[ws.instanceId.guid];
	}
}
...
function showStatus() {
	console.log(`Probation: ${serverStatus.probationSocketHolder.sockets.length} Verified: ${serverStatus.verifiedSocketHolder.sockets.length} Promoted: ${promoted} Timed out: ${timedOut} Bad net: ${badnet} Ping count: ${pingCount}`);
