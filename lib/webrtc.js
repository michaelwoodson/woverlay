...
class WebRTC {
	...
	start(isInitiator, peerId, instanceId, signaller, timestamp, websocketProxy, websocketProxyInstance) {
		...
		// Some examples use thsi event to trigger connection.pc.createOffer, but this
		...
		connection.pc.oniceconnectionstatechange = () => {
			if (connection.pc.iceConnectionState === 'disconnected' || connection.pc.iceConnectionState === 'closed') {
	...
	handleSignal(peerId, instanceId, envelope, signaller) {
		...
		if (envelope.timestamp != connection.timestamp) {
			if (connection.timestamp > envelope.timestamp) {
				...
				this.start(false, peerId, instanceId, signaller, envelope.timestamp);
				connection = this.getConnection(peerId, instanceId);
			}
		}
		...
		if (envelope.timestamp != connection.timestamp) {
			...
			if (envelope.message.candidate) {
				try {
					connection.pc.addIceCandidate(new badapter.RTCIceCandidate(envelope.message.candidate));
				} catch (e) {
					console.log('glitched message: ' + JSON.stringify(envelope));
					console.log('icy error: ' + e.message);
				}
			} else {
	...
	/**
	 * Message handler for test messages.	Can be replaced for different
	 * behavior.
	...
	setupChannel(connection) {
		connection.channel.onmessage = (event) => {
			if (event.data.length === 0) {
				...
				if ('broadcast' === envelope.action) {
					this.messageHandler(envelope.message);
				} else if ('markUsed' === envelope.action) {
				...
				try {
					...
					setTimeout(() => {
						try {
							...
							console.log(`failed to send to (retried): ${envelope.to} closed: ${connection.closed} iceState: ${connection.pc.iceConnectionState} readyState: ${connection.channel.readyState} signalingState: ${connection.pc.signalingState} msg: ${e2.message}`);
						}
					}, 5000);
				}
			} else {
	...
	removeConnection(connection) {
		...
		this.overlay.closed(connection.peerId);
		this.updateListeners();
	}
