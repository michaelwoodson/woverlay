...
class WebRTC {
	...
	start(isInitiator, peerId, instanceId, signaller, timestamp, websocketProxy, websocketProxyInstance) {
		...
		connection.pc.onidpassertionerror = logError;
		connection.pc.onidpvalidationerror = logError;
		if (isInitiator) {
	...
	getOpenConnections() {
		let result = [];
		this.connections.filter(c => this.active(c.peerId)).forEach(connection => {
	...
	send(envelope) {
		...
		if (connection && connection.channel) {
			if (connection.channel.readyState === 'open') {
				...
				console.log('cannot send: ' + connection.channel.readyState + ' ' + envelope.to);
			}
		} else {
...
/**
 * Log an error from the underlying webrtc framework.
 * @private
 */
