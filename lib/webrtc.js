		...
		if (envelope.timestamp != connection.timestamp) {
			...
			if (envelope.message.candidate) {
				try {
					...
					console.log(`isOveraly: ${connection.signalingChannel.isOverlay()} signalingState: ${connection.pc.signalingState} iceConnectionState: ${connection.pc.iceConnectionState} message: ${e.message}`);
				}
			} else {
